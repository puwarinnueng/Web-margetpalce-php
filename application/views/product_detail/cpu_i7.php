<?php
defined('BASEPATH') or exit('No direct script access allowed');
?>
<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="utf-8">
  <title>INTEL LGA1200 CORE I7-10700 3.8 GHz</title>
</head>

<body>
  <div class="container">
    <div class="row row1">
      <div class="col-md-8">
        <div1 class="card_product_detail">
          <div class="img_detail">
            <img src="<?php echo base_url('img/cpu3.png'); ?>">
          </div>
          <div class="infos">
            <h1 class="name_detail">CPU (ซีพียู) INTEL LGA1200 CORE I7-10700 3.8 GHz </h1><br>
            <p>- แบรนด์ INTEL </p>
            <p>- CORES : 8</p>
            <p>- THREADS : 16</p>
            <p>- รองรับ Intel Optane Memory</p>
            <p>- ใช้งานกับ Intel 400 Series chipset-based mainboard</p>
            <br>
            <h4 class="guarantee"><i class="fa fa-certificate" aria-hidden="true" style="font-size:20px;color:#FACC2E"></i> ประกัน 3 ปี</h4><br><br>
            <h3 class="price_detail"> ราคา 15,000 บาท</h3>
            <br>
            <p class="availbilty">AVAILABILITY : In Stock</p>
            <br>
            <button class="buttons_detail"><a href="<?php echo site_url('cart/cpu_i7'); ?>"><i class="fa fa-shopping-cart" aria-hidden="true"></i> เพิ่มใส่รถเข็น </a></button>
          </div>
      </div>

      <div class="col-md-4">
        <div class="card_right">
          <table class="card_right_table">
            <tr>
              <td><i class="fas fa-thumbs-up" id="thumbs_up_icon1"></i> รับประกันสินค้าแท้ 100%</td>
            </tr>
            <tr>
              <td><i class="fas fa-shipping-fast" id="truck_icon"></i> ส่งฟรีทั่วไทย
                <p style="font-size: 10px; ">ช้อปครบ 3000.- ขึ้นไป</p>
              </td>
            </tr>
            <tr>
              <td><i class="fa fa-exchange" aria-hidden="true" id="exchange_icon"></i> สามารถเปลี่ยนสินค้าได้
                <p style="font-size: 10px;"> ภายใน 7 วัน</p>
              </td>
            </tr>
            <tr>
              <td class="pay"><i class="fas fa-credit-card" id="creditcard_icon1"></i> ชำระเงินได้หลายช่องทาง
                <p style="font-size: 10px;">- เก็บเงินปลายทาง</p>
                <p style="font-size: 10px;">- โอนผ่านทางธนาคาร/ATM</p>
              </td>
            </tr>
          </table>
        </div>
      </div>
    </div>

    <div class="row row2">
      <div class="col-md-8">
        <div class="discription">
          <h3>รายละเอียดสินค้า</h3>
          <div class="content_discription">
            <table class="discription_table">
              <tr>
                <td>Brand </td>
                <td>INTEL</td>
              </tr>
              <tr>
                <td>Model </td>
                <td>INTEL CORE I7 - 10700K</td>
              </tr>
              <tr>
                <td>Socket </td>
                <td>1200 Comet Lake 400 Series</td>
              </tr>
              <tr>
                <td>CPU Core / Thread </td>
                <td>8/16</td>
              </tr>
              <tr>
                <td>Frequency </td>
                <td>3.80 GHz</td>
              </tr>
              <tr>
                <td>Turbo Frequency </td>
                <td>5.10 GHz</td>
              </tr>
              <tr>
                <td>Cache </td>
                <td>16 MB Intel Smart Cache</td>
              </tr>
              <tr>
                <td>Integrated Graphics </td>
                <td>Intel UHD Graphics 630</td>
              </tr>
              <tr>
                <td>TDP </td>
                <td>95 W</td>
              </tr>
            </table>
          </div>
        </div>
      </div>
    </div>
  </div>


</body>

</html>